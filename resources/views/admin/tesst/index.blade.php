...
@section("content")
    ...
    <table class="border-separate border-2 text-center border-gray-500 mt-3" style="width: 100%">
        <thead>
        <tr>
            <th class="px-4 py-2">{{ __("Pregunta") }}</th>
        </tr>
        </thead>
        <tbody>
            @forelse($tesst as $test)
                <tr>
                    <td class="border px-4 py-2">{{ $tesst->test }}</td>
                    <td class="border px-4 py-2">
                        ...
                        <a
                            ...
                            onclick="event.preventDefault();
                                document.getElementById('delete-tessts-{{ $tesst->id }}-form').submit();"
                        >{{ __("Eliminar") }}
                        </a>
                        <form id="delete-preguntas-{{ $tesst->id }}-form" action="{{ route('admin.tesst.destroy', ['teest' => $test]) }}" method="POST" class="hidden">
                            @method("DELETE")
                            @csrf
                        </form>
                    </td>
                ...
                <tr>
                    <td colspan="4">
                        <div class="bg-red-100 text-center border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                            <p><strong class="font-bold">{{ __("No hay preguntas") }}</strong></p>
